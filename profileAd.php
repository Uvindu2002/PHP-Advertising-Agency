		...
		<div class="container">
            ...
            if (isset($_FILES["image"])) {
                ...
                if (!in_array($file_ex, $allowed_image_extensions)) {
                    ...
                    $image_query = "SELECT * FROM `profileAd_image` WHERE advertiser_email = '".$_SESSION['u']['email']."'";
                    $image_rs = mysqli_query($conn, $image_query);
                    $image_num = mysqli_num_rows($image_rs);

                    if ($image_num == 1) {
                        $update = "UPDATE `profileAd_image` SET path_url = '$file_name' WHERE advertiser_email = '".$_SESSION['u']['email']."'";
                        $update_rs = mysqli_query($conn, $update);
                    } else {
                        $insert = "INSERT INTO `profileAd_image` (path_url, advertiser_email) VALUES ('$file_name', '".$_SESSION['u']['email']."')";
            ...
            <?php
                ...
                if (empty($fname) || empty($lname) || empty($mobile)) {
                    ...
                    $query = "UPDATE `advertisers` SET fname = '$fname', lname = '$lname', mobile = '$mobile' WHERE email = '$email'";
                    $rs = mysqli_query($conn, $query);
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="row mt-4">
                            ...
                            <div class="col8 margintop25">
                                ...
                                <input type="text" class="fcontrol" value="<?php echo ($data["mobile"]); ?>" name="mobile" />
                            </div>
